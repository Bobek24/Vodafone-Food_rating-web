...
function SurveyForm({ meal, userEmail, token, onClose }) {
  const [formData, setFormData] = useState({
    ...
    porce: '',
    teplota: '',
    starRating: 0,  // nově pro hvězdičkové hodnocení
    priplatek: ''
  });
  const [hasSubmitted, setHasSubmitted] = useState(false);

  useEffect(() => {
    // Zkontrolujeme, jestli už je hodnocení pro daného uživatele a jídlo v localStorage
  ...
  const handleSubmit = (e) => {
    ...
    // V produkci byste volali např. POST /api/rating
    // a backend by ověřil, jestli už userEmail + mealId existuje.
    ...
    savedResponses.push({
      ...
      // Uložíme hvězdičkové hodnocení jako "vzhled jídla"
